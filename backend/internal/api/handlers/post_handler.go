...
func (h *PostHandler) GetPosts(c *gin.Context) {
	...
	switch sortBy {
	case "hot":
		// Hot algorithm: consider vote_count and time
	...
	for _, post := range posts {
		response = append(response, h.toPostResponse(post, userID.(uint)))
	}
...
func (h *PostHandler) GetPost(c *gin.Context) {
	...
	c.JSON(http.StatusOK, h.toPostResponse(post, userID.(uint)))
}
